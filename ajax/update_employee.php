...
try {
    ...
    if ($controller->emailExists($data['email'], $employeeId)) {
        send_json(['success' => false, 'message' => 'Email already exists']);
    }

    // Update employee
    $result = $controller->updateEmployee($employeeId, $data);
    
    // Handle allowances and deductions if employee was updated successfully
    if ($result['success']) {
        // Process allowances - update assignments
        $selectedAllowances = isset($_POST['allowances']) && is_array($_POST['allowances']) ? $_POST['allowances'] : [];
        $allowances = [];
        
        foreach ($selectedAllowances as $allowanceId) {
        ...
        // Remove unselected allowances and assign/update selected ones
        $controller->removeUnselectedAllowances($employeeId, array_keys($allowances));
        if (!empty($allowances)) {
            $controller->assignAllowancesToEmployee($employeeId, $allowances);
        }
        
        // Process deductions - update assignments
        $selectedDeductions = isset($_POST['deductions']) && is_array($_POST['deductions']) ? $_POST['deductions'] : [];
        $deductions = [];
        ...
        // Remove unselected deductions and assign/update selected ones
        $controller->removeUnselectedDeductions($employeeId, array_keys($deductions));
        if (!empty($deductions)) {
        ...
        if (isset($_SESSION['user_id'])) {
            ...
            $logger->logEmployeeAction($_SESSION['user_id'], 'updated', $employeeId, "Employee information updated");
        }
    }
